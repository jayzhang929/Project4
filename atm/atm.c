	            ...
	            printf("Usage: begin-session <user-name>\n");
			 	return;		
					...
					//User has been verified and can now get to conduct other transactions
						...
						if (token == NULL || !withdraw_check(token)) {
					             printf("Usage: withdraw <amt>\n");
						     return;
						    }else {      

						         if(strcmp(token,"4294967295") > 0) {
						         	printf("Insufficient funds\n");
						         	return;
						         }
						        char *ptr; 
						        unsigned int amount =strtoul(token, &ptr, 10);
		                               
		 
			                    memset(balbuf,0x00,strlen(balbuf));
					            memset(call,0x00,strlen(call));
						        memset(cipher,0x00,strlen(cipher));			
							    sprintf(balbuf,"%s %s %u","withdraw",user_name,amount);
